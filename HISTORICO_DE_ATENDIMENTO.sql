...
SELECT  
			...
			k.FKDAT as 'DATA FAT',
			p.TXJCD as 'COD_IBGE'
		FROM (SELECT REGIO, INCO2, FKDAT, FKART, FKSTO, SFAKN, KUNRG, VBELN  FROM unifort_prod.dbo.VBRK WHERE FKDAT BETWEEN '01/11/2022' AND '31/12/2022' and FKART = 'F2B' and FKSTO = '' and SFAKN = '') k
		left join (SELECT NETWR, KZWI1, KZWI4, FKIMG, GSBER, VGBEL, AUBEL, MATNR, ARKTX, TXJCD ,VBELN FROM SAP_VBRP.dbo.VBRP WHERE FBUDA BETWEEN '01/11/2022' AND '31/12/2022' ) p ON (p.VBELN = k.VBELN)
