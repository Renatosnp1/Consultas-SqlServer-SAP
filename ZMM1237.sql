...
SELECT z.Material FROM ZMM1237 z WHERE z.DATA_REL = '01/12/2022' AND z.CL != 'DS';
...
SELECT		YEAR(MAX(DATA_REL)) AS ANO,
		...
		FROM ZMM1237
	WHERE  DATA_REL in ('31/01/2022', '28/02/2022', '31/03/2022', '30/04/2022', '31/05/2022', '30/06/2022', '31/07/2022', '31/08/2022', '30/09/2022', '27/10/2022', '30/11/2022', '01/12/2022') 
...
SELECT TOP (10) * FROM view_zmm1237_mensal 
...
DECLARE @dt DATE;
SET @dt = (SELECT MAX(DATA_REL) FROM ZMM1237 WHERE DATA_REL = '29/11/2022')
SELECT [Material], [U#L] FROM ZMM1237 WHERE DATA_REL = @dt AND Material = '5000.275.022'
GO


SELECT p1.* FROM (SELECT  DATA_rel,
		MATERIAL,
		DATENAME(dw, DATA_rel) [DIA DA SEMANA],
	    SUM(ESTOQUE_TOTAL) ESTOQUE_TOTAL
	FROM SAP_ESTOQUE.dbo.SQVI_ESTOQUE WHERE DATA_rel > '01/11/2022' AND MATERIAL = '5000.275.022'
...
SELECT	p1.MATERIAL,
		FORMAT(GETDATE(), 'dd/MM/yyyy') [DATA ATUALIZACAO REL],
		COUNT(p1.MATERIAL) [DIAS DE ESTOQUE],
		AVG(p1.ESTOQUE_TOTAL) [MEDIAESTOQUE DIA],
		MIN(p1.ESTOQUE_TOTAL) [ESTOQUE MIN],
		MAX(p1.ESTOQUE_TOTAL) [ESTOQUE MAX]
	FROM (SELECT  DATA_rel,
		MATERIAL, 
	    SUM(ESTOQUE_TOTAL) ESTOQUE_TOTAL
		FROM SAP_ESTOQUE.dbo.SQVI_ESTOQUE WHERE DATA_rel > '01/10/2022' AND MATERIAL in ('5000.275.014')
		GROUP BY DATA_rel, MATERIAL) p1
