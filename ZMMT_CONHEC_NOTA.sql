...
DELETE  FROM SAP_DIV.dbo.ZMMT_CONHEC_NOTA
 WHERE ORDEM_VENDA in ('')

USE query_dw
GO
CREATE VIEW view_conhec_nota
as 
	SELECT	v1.ORDEM_VENDA OV_DIR,
			v2.ORDEM_VENDA OV_TRANS,
			v1.CONHECIMENTO CONHEC_DIR,
			v1.ID_PARCEIRO ID_PAR_DIR,
			v1.NUMERO_NF NUMERO_NF_DIR,
			v1.TIPO_PARCEIRO DIRETO,
			v2.CONHECIMENTO CONHEC_TRANS,
			v2.ID_PARCEIRO ID_PAR_TRANS,
			v2.NUMERO_NF NUMERO_NF_DIR_TRANS,
			v2.TIPO_PARCEIRO TRANSFERENCIA
		FROM (SELECT cn.CONHECIMENTO, cn.ID_PARCEIRO, cn.VALOR_TOT_NOTA, cn.ORDEM_VENDA ,cn.TIPO_PARCEIRO, cn.NUMERO_NF
		FROM SAP_DIV.dbo.ZMMT_CONHEC_NOTA cn WHERE cn.TIPO_PARCEIRO = 'C') v1
		full outer join (SELECT c.CONHECIMENTO, c.ID_PARCEIRO, c.VALOR_TOT_NOTA, c.ORDEM_VENDA ,c.TIPO_PARCEIRO, c.NUMERO_NF 
					FROM SAP_DIV.dbo.ZMMT_CONHEC_NOTA c WHERE c.TIPO_PARCEIRO = 'B') v2 ON (v1.ORDEM_VENDA = v2.ORDEM_VENDA)
GO

DROP TABLE t_conhec_nota

SELECT p1.* INTO t_conhec_nota FROM (SELECT	
		v1.ORDEM_VENDA OV_DIR,
		v1.VALOR_TOT_NOTA VLR_TT_NF_D,
		v1.DATA_EMISS_CON DT_CON_D,
		v1.CONHECIMENTO CONHEC_D,
		v1.ID_PARCEIRO ID_PAR_D,
		v1.NUMERO_NF NF_D,
		v1.TIPO_PARCEIRO DISTRIBUICAO,
		v2.ORDEM_VENDA OV_TR,
		v2.VALOR_TOT_NOTA VLR_TT_NF_TR,
		v2.DATA_EMISS_CON DT_CON_TR,
		v2.CONHECIMENTO CONHEC_TR,
		v2.ID_PARCEIRO ID_PAR_TR,
		v2.NUMERO_NF NF_TR,
		v2.TIPO_PARCEIRO TRANSFERENCIA,
		c.FATURA
	FROM (SELECT cn.CONHECIMENTO, cn.ID_PARCEIRO, cn.VALOR_TOT_NOTA, cn.ORDEM_VENDA ,cn.TIPO_PARCEIRO, cn.NUMERO_NF, cn.DATA_EMISS_CON
	FROM SAP_DIV.dbo.ZMMT_CONHEC_NOTA cn WHERE cn.DATA_EMISS_CON >= '01/01/2020' AND cn.TIPO_PARCEIRO = 'C') v1
	full outer join (SELECT cn.CONHECIMENTO, cn.ID_PARCEIRO, cn.VALOR_TOT_NOTA, cn.ORDEM_VENDA ,cn.TIPO_PARCEIRO, cn.NUMERO_NF, cn.DATA_EMISS_CON 
	FROM SAP_DIV.dbo.ZMMT_CONHEC_NOTA cn WHERE cn.DATA_EMISS_CON >= '01/01/2020' AND cn.TIPO_PARCEIRO = 'B') v2 ON (v1.ORDEM_VENDA = v2.ORDEM_VENDA)
	LEFT JOIN (SELECT c.FATURA, c.CONHECIMENTO FROM SAP_DIV.dbo.ZMMT_FAT_CONHE c) c ON (v2.CONHECIMENTO = c.CONHECIMENTO)) p1

SELECT * FROM t_conhec_nota
